...
def main():
  ...
  output_dir = args.output_dir or f'results/opt_500_1000_1/{image_list_name}'
  ...
  #真实图片的自编码器输出
  real_encode = tf.reshape(E.get_output_for( real_image, is_training = False), latent_shape)
  #真实图片的VGG16输出
  real_feat = perceptual_model( real_image_255 )

  init_latent_X = tf.assign(latent_X, real_encode)
  ...
  encode_rec = tf.reshape(E.get_output_for( image_rec, is_training = False), latent_shape)

  #真实图片和重建图片基于逐个像素的误差
  Loss_enc = tf.reduce_mean( tf.square( encode_rec - real_encode ), axis = [1,2])
  Loss_pix = tf.reduce_mean( tf.square( image_rec - real_image ), axis = [1,2,3]) + weight_for_enc*Loss_enc
  #真实图片和重建图片基于VGG16提取特征的误差
  Loss_feat = tf.reduce_mean( tf.square( feat_rec - real_feat ), axis = [1]) + weight_for_enc*Loss_enc
  
  #损失函数为三者误差总和
  Loss = Loss_pix + weight_for_feature*Loss_feat + weight_for_enc*Loss_enc
  ...
  #optimizer_enc = tf.train.AdamOptimizer(enc_learning_rate)
  #enc_train_op = optimizer_enc.minimize(Loss_enc, var_list=[latent_X])
  ...
  for img_idx in tqdm(range(0, len(image_list), batch_size), leave=False):
    ...
    for step in tqdm(range(1, epochs+1), leave=False):
      ...
      #sess.run(enc_train_op, {real_image:inputs})
