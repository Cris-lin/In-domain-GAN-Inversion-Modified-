...
def main():
    ...
    sample_image_list = f'to_score/sample.list'
    ...
    print('重建FID为：', np.real(dist), 'MSE为', np.mean(np.square(inv_img - real_img)))

    image_list = []
    with open(sample_image_list, 'r') as f:
        for line in f:
            image_list.append(line.strip())
    ...
    images = np.asarray(images, dtype=np.float32)
    #inputs = images.astype(np.float32) / 255 * 2.0 - 1.0
    sample_img = images.astype(np.float32) / 255 * 2.0 - 1.0

    sample_activations = inception.run(images, assume_frozen=True)
    mu_sample = np.mean(sample_activations, axis = 0)
    sigma_enc = np.cov(sample_activations, rowvar=False)

    m = np.square(mu_sample - mu_real).sum()
    ...
    print('sample FID为：', np.real(dist), 'MSE为', np.mean(np.square(sample_img - real_img)))
